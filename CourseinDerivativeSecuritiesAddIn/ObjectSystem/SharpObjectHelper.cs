...
namespace DerivativeSecuritiesAddIn.ObjectSystem {
    public static class SharpObjectHelper {
        ...
        internal const string IDPREFIX = "¡ì";
        ...
        public static object CreateObj(object[,] value) {
            ...
            if (rid == null || rid is Range || string.IsNullOrWhiteSpace(rid.ToString()) || rid.ToString().StartsWith(IDPREFIX))
                return "Invalid Id";
            var id = $"{IDPREFIX}{rid.ToString().ToUpper()}";
        ...
        internal static SharpObject Find(string id) =>
            !string.IsNullOrWhiteSpace(id) && Env.TryGetValue(id, out var so) ? so : null;
    }
}